...
- **연결 커넥터**:
  ...
  - ~~Gmail~~ — 클라우드 루틴에 붙일 수 없음(Anthropic 호스팅 연동이라 MCP URL 없음).
    자동 이메일 발송 대신 리포트를 Drive의 날짜별 폴더에 저장한다.

## 2) 루틴 프롬프트 (그대로 붙여넣기)
> 발송 없음 — 결과는 Drive `place-scan/reports/{날짜} — {사례}/` 폴더에 저장된다.
...
3. STYLE.md 문체로 쓴 뒤 /yunmun 윤문 교정 스킬을 적용해 한글 표현을 다듬어
   최종 한국어 리포트를 완성한다.
4. (이메일 발송 없음) 클라우드 루틴에는 Gmail을 붙일 수 없어 발송하지 않는다.
   리포트는 아래 Drive 폴더에 저장해 아카이브한다.
...
7. 재무필터를 통과한 기업이 0건이면: 폴더명을 "{오늘 날짜} — (검증 필요만)"으로 하고
   검증 필요 목록만 저장하며, seen는 갱신하지 않는다.
